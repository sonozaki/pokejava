	...
	public static void main(String[] args) {
		...
		try {
			if (args[0].equals("--help")) {
				...
				System.out.println(
						"Pokejava necesita dos ficheros pkm para obtener las propiedades de cada pokemon, ejemplo:");
				System.out.println("");
		...
		while (pokemon1.getPv() > 0 && pokemon2.getPv() > 0) {
			...
			if (!turno) {
				if (pokemon1.getEstado().equals("")) {
					...
					System.out.print("\n[pv:" + pokemon1.getPv() + "][" + pokemon1.getEstado() + "]["
							+ pokemon1.getNombre() + "] --> ");
				}
			} else {
				if (pokemon2.getEstado().equals("")) {
					...
					System.out.print("\n[pv:" + pokemon2.getPv() + "][" + pokemon2.getEstado() + "]["
							+ pokemon2.getNombre() + "] --> ");
				}
			...
			// Gestionamos los comandos, también gestionamos el turno cambiando el valor
			// del booleano turno
			switch (orden) {
			case "atacar":
				if (!turno)
			...
			case "informacion":
				if (!turno)
					pokemon1.informacion(pokemon2);
				else
					pokemon2.informacion(pokemon1);
				break;
			default:
